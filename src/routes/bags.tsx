          ...
          {bags.map((b, i) => (
            ...
            >
              <div className="aspect-square overflow-hidden bg-white">
                <img src={b.img} alt={b.name} loading="lazy" className="w-full h-full object-contain p-4 transition-transform duration-500 group-hover:scale-105" />
              </div>
              <div className="p-4 md:p-5 flex flex-col gap-3 flex-1">
                ...
                <div className="mt-auto pt-1">
                  <OrderDialog product={`${b.name} (${b.spec})`} price={b.price} triggerClassName="w-full inline-flex items-center justify-center gap-2 px-3 py-2 rounded-full bg-accent text-accent-foreground text-xs md:text-sm font-medium hover:opacity-90 transition" triggerLabel="Place Order" />
                </div>
              </div>
            </motion.div>
          ))}
